    ...
    public double[] updatePlanes(ArrayList<Plane> planes, int round, double[] bearings) {
        ...
        for (int i = 0; i < planes.size(); i++) {
            ...
            if (round >= plane.getDepartureTime() && plane.getBearing() == -1) {
                ...
                if (!lastTakeoffTime.containsKey(destination) || round >= lastTakeoffTime.get(destination) + 4) {
                    // Allow takeoff
        ...
        if (planesInAir.size() > 1) {
            ...
            if (simulation.getReason() == 4 && simulation.getRound() - round <= 15) {
                ...
                for (int i = 0; i < planesInAir.size(); i++) {
                    ...
                    if (Math.abs(newBearing - plane.getBearing()) > 10 && round > 10) {
                        bearings[index] = plane.getBearing() - 10;
                    } else {
                        bearings[index] = newBearing;
                    }
                }
            }
        }
        ...
        for (int i = 0; i < bearings.length; i++) {
            if (bearings[i] >= 0 && bearings[i] != -1 && bearings[i] != -2) {
                bearings[i] = bearings[i] % 360;
            } else if (bearings[i] < 0 && bearings[i] != -1 && bearings[i] != -2) {
                bearings[i] = ((bearings[i] % 360) + 360) % 360;
            }
